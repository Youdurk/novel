<template>
    ...
    <el-row :gutter="10">
  <el-col :span="8" v-for="item in xianxia" :key="item.id" >
    <div style="width: 350px;height: 156px;cursor: pointer;">
      <img :src="item.bookUrl" class="image"  @click="$router.push('/briefbook?id='+item.id+'&heat='+(item.heat+1))"/>
...
export default{
    ...
    methods:{
      load(){
        this.request.get("/novel/popular/xianxia").then(res =>{
          console.log(res.data)
          this.xianxia=res.data
        })
        this.request.get("/novel/page/xianxia",{params:{
